...
# Added 'r' to make it a raw string, preventing Python from misinterpreting escape sequences
HTML_CONTENT = r""" 
<!DOCTYPE html>
<html lang="en">
<head>
...
# Your API key from the screenshot is used here.
# Ensure this is your correct and active key.
# For better security, consider environment variables in the long run.
os.environ["GEMINI_API_KEY"] = "AIzaSyAZYNPDwwq6m4AZBx1eZVOtFoUBcwunHLE"  # This should match your hello3.py line 506
...
try:
    ...
    # For now, we'll log and continue, but file operations might fail later.
    ...
    try:
        ...
        for i, image in enumerate(images):
            try:
                ...
                # Optionally, continue to next page or append an error marker
        logger.info(f"OCR extraction completed for {file_path}. Length: {len(text)}")
        return text
    except Exception as e: # Catch broader exceptions from pdf2image or pytesseract setup
...
def load_pdf(file_path):
    ...
    try:
        ...
        if not text.strip() or len(text.strip()) < 100: # Threshold for "very little text"
            logger.warning(f"Extracted text from {file_path} via PyPDF is very short or empty. Attempting OCR for the entire document.")
            text_ocr = extract_text_with_ocr(file_path)
            if text_ocr: # Prefer OCR if it yields more, otherwise keep PyPDF's (even if short)
                text = text_ocr
...
def split_text(text: str):
    ...
    for chunk in split_text_list:
        chunk = chunk.strip()
        if chunk: # Ensure chunk is not empty after stripping
            # Simple heuristic: if a "line" (after splitting by \n) is too long, 
            # it might be a paragraph without newlines. We won't split it further here
            # as Gemini embedding model can handle reasonable chunk sizes.
            # The main goal is to split distinct paragraphs or text blocks.
            final_chunks.append(chunk)
    
    # Filter for chunks with a minimum number of words to be meaningful
    min_words_per_chunk = 5 
    meaningful_chunks = [c for c in final_chunks if len(c.split()) >= min_words_per_chunk]
    
    if not meaningful_chunks and final_chunks: # If filtering removed everything but there was *some* text
        logger.warning(f"Text splitting resulted in no chunks meeting min_words criteria ({min_words_per_chunk}), but original split had {len(final_chunks)} items. Consider reviewing PDF content or splitting logic if this is unexpected.")
        return final_chunks # Return original non-empty chunks if filtering is too aggressive
...
class GeminiEmbeddingFunction(chromadb.EmbeddingFunction):
    def __call__(self, input: List[str]) -> List[List[float]]: 
        ...
        if not gemini_api_key:
            logger.error("Gemini API Key not provided for embedding.")
            raise ValueError("Gemini API Key not provided for embedding function.")
        try:
        ...
        batch_size = 100 # Max batch size for Gemini embed_content
        
        input_texts: List[str] = list(input) # Ensure it's a list for slicing

        for i in range(0, len(input_texts), batch_size):
            batch = input_texts[i:i+batch_size]
            if not batch: # Should not happen with proper range stepping but good for safety
                continue
            try:
                ...
                batch_embeddings = results.get('embedding') # Use .get for safety
                ...
                # Ensure the structure is List[List[float]]
                ...
                # If the batch had 1 item, Gemini might return a single list [float, float,...]
                # instead of [[float, float,...]]. We need to ensure it's List[List[float]].
                if len(batch) == 1 and batch_embeddings and isinstance(batch_embeddings[0], float):
                     all_embeddings.append(batch_embeddings) # It was a single embedding vector for a single input text
                elif all(isinstance(emb, list) for emb in batch_embeddings): # Expected: list of lists
                ...
                # Propagate the error to signal failure in embedding
                raise RuntimeError(f"Gemini embedding failed for a batch: {e}") from e
        
        if len(all_embeddings) != len(input_texts):
            logger.error(f"Mismatch in number of embeddings ({len(all_embeddings)}) vs input texts ({len(input_texts)}).")
...
def create_chroma_db(documents: List[str], name: str):
    ...
    try:
        # get_or_create_collection is idempotent and preferred
        ...
        # This block might be redundant if get_or_create_collection is robust, but as a fallback:
        try:
            logger.info(f"Attempting explicit creation of collection '{name}' as fallback.")
            db_collection = chroma_client.create_collection(name=name, embedding_function=GeminiEmbeddingFunction())
            ...
            logger.error(f"Fatal error creating or getting collection '{name}' after fallback: {e_create_final}")
            raise RuntimeError(f"ChromaDB collection handling failed for '{name}': {e_create_final}") from e_create_final

    if documents:
        doc_ids = [str(i) for i in range(len(documents))] # Simple sequential IDs
        try:
            logger.info(f"Adding/updating {len(documents)} documents in collection '{name}'. First doc snippet: '{documents[0][:50]}...'")
            db_collection.add(documents=documents, ids=doc_ids) # Upserts by default if IDs exist
...
def load_chroma_collection(name: str):
    ...
    except chromadb.errors.CollectionNotDefinedError: # More specific error
        logger.error(f"Collection '{name}' does not exist at '{chroma_path}'.")
        raise ValueError(f"Collection '{name}' not found. Please ensure the PDF was uploaded and processed successfully.")
    except Exception as e:
...
def get_relevant_passage(query: str, db_collection, n_results: int = 5):
    ...
    try:
        ...
        # ChromaDB 0.5.x query result structure:
        # results = {'ids': [[]], 'distances': [[]], 'metadatas': [[]], 'embeddings': None, 'documents': [[]], 'uris': None, 'data': None}
        # We need results['documents'][0] which is a list of document strings.
        
        if results and results.get('documents') and isinstance(results['documents'], list) and results['documents'] and isinstance(results['documents'][0], list) and results['documents'][0]:
            # results['documents'][0] is the list of retrieved document strings for the first query
        ...
        logger.warning(f"No relevant passages found in DB for query: '{query}' in collection '{db_collection.name}'. Query result: {results}")
...
def upload_file_route(): 
    ...
    if not file_obj or not allowed_file(file_obj.filename):
        logger.warning(f"Upload API: Invalid file type or no file object for '{file_obj.filename if file_obj else 'N/A'}'")
    ...
    # Sanitize base_filename for collection name (alphanumeric, underscore, hyphen)
    # Max length for collection names in Chroma is 63 chars.
    # Also, must start and end with alphanumeric, not contain "..", etc.
    sanitized_base = re.sub(r'[^a-zA-Z0-9_-]', '_', base_filename)
    sanitized_base = re.sub(r'_+', '_', sanitized_base).strip('_') # Remove multiple and leading/trailing underscores
    if not sanitized_base: sanitized_base = "default_pdf" # Handle empty after sanitization
    collection_name_prefix = "rag_"
    max_base_len = 63 - len(collection_name_prefix) - 5 # 5 for potential suffix/hash
    collection_name = f"{collection_name_prefix}{sanitized_base[:max_base_len]}"
    
    # Ensure collection name starts and ends with alphanumeric if it's short
    if len(collection_name) > 2 and (not collection_name[0].isalnum() or not collection_name[-1].isalnum()):
         collection_name = "c" + collection_name.strip('_')[:60] + "c" # simple fix
    if len(collection_name) < 3: # Min length for Chroma collection name
        collection_name = collection_name + "xxx" 
    collection_name = collection_name[:63] # Final trim
    ...
    try:
        ...
        if not chunked_text:
            logger.error(f"Upload API: Text extracted from '{filename}', but no usable chunks after splitting.")
            return jsonify({"error": f"PDF text from {filename} was extracted but resulted in no usable content chunks."}), 500
        
        logger.info(f"Upload API: Successfully chunked text from '{filename}'. Number of chunks: {len(chunked_text)}")
        
        create_chroma_db(chunked_text, collection_name)
        
        return jsonify({"message": "File uploaded and processed successfully", "filename": filename, "collection_name": collection_name}), 200

    except RuntimeError as re: # Catch runtime errors from helper functions
        logger.error(f"Upload API: Runtime error during processing for '{filename}': {re}")
        return jsonify({"error": f"Processing error: {str(re)}"}), 500
    except Exception as e:
        logger.exception(f"Upload API: Unexpected error during file upload/processing for '{filename}': {e}")
...
def ask_query_route():
    try:
        ...
        # The string "No relevant passage found in the document." is a specific signal from get_relevant_passage
        if passage == "No relevant passage found in the document.":
            logger.warning(f"Ask-Query API: No relevant passage found for query '{query}' in '{collection_name}'. Responding to user directly.")
            # This answer is what the user will see directly if no relevant context is found.
            return jsonify({"answer": f"I couldn't find specific information related to your query \"{query}\" in the uploaded document. You could try rephrasing your question, or ensure the document contains the details you're looking for."})
        elif "Error retrieving passages" in passage: # Indicates an error during DB query
             logger.error(f"Ask-Query API: Error retrieving passages for query '{query}' in '{collection_name}'. Passage content: {passage}")
        ...
        gemini_api_key = os.getenv("GEMINI_API_KEY")
        if not gemini_api_key:
            logger.error("Ask-Query API: Gemini API Key not found for query execution.")
            return jsonify({"error": "Gemini API Key not configured on server."}), 500
        genai.configure(api_key=gemini_api_key)
        
        model = genai.GenerativeModel('gemini-pro')
        
        logger.debug(f"Ask-Query API: Generating content with Gemini. Prompt snippet: {prompt[:300]}...")
        response = model.generate_content(prompt)
        
        # Rigorous check for valid response content
        answer_text = ""
        try:
            answer_text = response.text # Preferred way to get text if available
        except ValueError as ve_text: # .text can raise ValueError if content is blocked
            logger.warning(f"Ask-Query API: Accessing response.text failed: {ve_text}. Checking prompt_feedback.")
            # Fall through to check prompt_feedback
        ...
        else: # No text from response.text, check feedback
            if hasattr(response, 'prompt_feedback') and response.prompt_feedback:
                feedback = response.prompt_feedback
                logger.error(f"Ask-Query API: Gemini response was empty or blocked. Prompt feedback: {feedback}")
                block_reason_message = "The model's response was blocked."
                if feedback.block_reason:
                    block_reason_message += f" Reason: {feedback.block_reason.name}." # .name for enum string
                if feedback.safety_ratings:
                     block_reason_message += f" Safety ratings: {[str(sr) for sr in feedback.safety_ratings]}"
                return jsonify({"error": f"{block_reason_message} Please try rephrasing your query or contact support if this persists."}), 500 # 500 or 400 depending on policy
            else: # No text and no feedback
                 logger.error(f"Ask-Query API: Gemini response has no text and no prompt_feedback. Query: {query}")
                 return jsonify({"error": "Failed to generate an answer from the AI model (empty or malformed response)."}), 500

    except ValueError as ve: # Catches DB not found from load_chroma_collection, or API key errors
        logger.error(f"Ask-Query API: ValueError: {ve}")
        return jsonify({"error": str(ve)}), 400 # 400 for bad request (e.g. bad collection name) or 404 if resource not found
    except RuntimeError as re:
        logger.error(f"Ask-Query API: Runtime error: {re}")
        return jsonify({"error": f"A runtime error occurred: {str(re)}"}), 500
    except Exception as e:
        logger.exception(f"Ask-Query API: Unexpected error for query '{data.get('query', 'N/A')}': {e}")
        # Check for specific Gemini API errors if possible by inspecting 'e'
        if "API key not valid" in str(e).lower() or "permission_denied" in str(e).lower(): 
            return jsonify({"error": "The Gemini API key is not valid or has insufficient permissions. Please check the server configuration."}), 500
...
if __name__ == '__main__':
    ...
    hardcoded_key_from_screenshot = "AIzaSyAZYNPDwwq6m4AZBx1eZVOtFoUBcwunHLE" # Key from your screenshot
    current_api_key = os.getenv("GEMINI_API_KEY")

    if current_api_key == hardcoded_key_from_screenshot:
        api_key_status = "SET (using the hardcoded value from the script)"
        logger.info(f"GEMINI_API_KEY is {api_key_status}.")
    elif current_api_key: 
        api_key_status = "SET (likely from a system environment variable, overriding script's hardcoded value)"
        logger.info(f"GEMINI_API_KEY is {api_key_status}.")
        logger.warning("An environment variable for GEMINI_API_KEY is set, which will be used instead of the one hardcoded in the script.")
    else: 
        api_key_status = "NOT SET. The hardcoded os.environ assignment did not take effect for getenv."
        logger.error(f"CRITICAL: GEMINI_API_KEY is {api_key_status}. The application will likely FAIL to connect to Gemini.")
        logger.error("Please ensure the line `os.environ[\"GEMINI_API_KEY\"] = \"YOUR_KEY\"` has your correct key, or set it as a system environment variable.")

    # For development, debug=True is useful. For production, set to False.
    # host='0.0.0.0' makes it accessible on your local network. Use '127.0.0.1' for local access only.
